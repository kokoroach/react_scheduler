  ...
  handleDBReponse(response) {
    ...
    const schedule = !appointments.length
      ...
      : appointments.reduce((currentSchedule, appointment) => {
          ...
          currentSchedule[slot_date] = !currentSchedule[slot_date]
            ? (currentSchedule[dateString] = Array(8).fill(false))
            : null;
            currentSchedule[slot_date] = Array.isArray(currentSchedule[dateString])
  ...
  renderAppointmentTimes() {
    if (!this.state.isLoading) {
      ...
      return slots.map((slot) => {
        ...
        return (
          <RadioButton
            label={time1.format("hh:mm") + " - " + time2.format("hh:mm A")}
            key={slot}
            value={slot}
            style={{
              marginBottom: 15,
